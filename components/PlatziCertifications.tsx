...
export function PlatziCertifications() {
  ...
  const mobileCertifications = hasPagination ? [...platziCertifications, ...platziCertifications] : platziCertifications
      ...
      <div className="mx-auto max-w-6xl animate-reveal">
        ...
        <div ref={sectionRef} className={`relative z-30 ${isSingleCertification ? "pb-8" : "pb-20"} sm:hidden`}>
          <div className={`relative left-1/2 w-screen -translate-x-1/2 overflow-visible px-4 ${isSingleCertification ? "pb-4" : "pb-10"} sm:left-auto sm:w-auto sm:translate-x-0 sm:px-0 sm:pb-0`}>
            <div className="relative overflow-visible">
              ...
              >
                {mobileCertifications.map((certification, index) => (
                  <div
                    key={`${certification.id}-${index}`}
                    ref={(node) => {
                      assignMobileCardRef(index, node)
                    }}
                    className={`${isSingleCertification ? "w-full max-w-[29rem] sm:mx-auto" : "min-w-[calc(100vw-2rem)] snap-center"} ${isSingleCertification ? "pb-6" : "pb-20"} sm:min-w-0 sm:pb-0 ${
        ...
        <div className="hidden sm:block">
          <div className={`overflow-visible ${isSingleCertification ? "pb-4" : "pb-10"} sm:pb-0`}>
            <div className={`${isSingleCertification ? "sm:flex" : "sm:grid"} sm:gap-5 sm:overflow-visible sm:pt-0 sm:pb-4 sm:snap-none ${desktopLayoutClassName}`}>
              {platziCertifications.map((certification, index) => (
                <div
                  key={certification.id}
                  className={`${isSingleCertification ? "sm:mx-auto" : ""} sm:min-w-0 sm:pb-0 ${
                    index % 3 === 1 ? "lg:translate-y-10" : index % 3 === 2 ? "sm:translate-y-6 lg:translate-y-20" : ""
                  } animate-reveal`}
                >
                  <PlatziCertificationCard certification={certification} />
